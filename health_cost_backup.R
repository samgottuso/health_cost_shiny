...
###Function that that takes healthcpost_values and puts it into graphs/tables
##(possible Grace ask on first iteration...)
...
#need to render a plot in reduction in Average BMI based on Age, Race, Gender, pop estimate, prediabetes pop, program attrition rate      x    <- faithful[, 2] 
      bins <- seq(min(x), max(x), length.out = input$bins + 1)
#Render 3 smaller plots-- total cases avoided, total costs avoided, ROI
